...
## 🔬 Full-code re-evaluation (2026-07-02, post-Round-2)
Three-way audit: **(A)** lib/data internal consistency (18 files) · **(B)** app-page hardcoded values vs canonical (21 pages) · **(C)** independent recomputation of 12 derived-calculation families (via scripts importing the actual modules).

**(A) Data layer:** no canonical contradictions in actual values. 1 fix: chp-sector `STATE_DISTRIBUTION` comment falsely claimed its 108k sum matches the 119k total → relabelled HIVE-indicative (relative shares only) + §6 chart title updated.

**(B) Page layer — 9 residual fixes:** homepage hero −45%→−37% + 1,786/3,226→2,222/3,521 · §2 cost KPI labels $310k/$560k→$284k/$450k · "18% of T1 report ESG"→"~12% of CHPs" (ESG urgency + sustainability stat) · ESG subtitle "65+ threshold"→HIVE benchmark wording · §13 CHP share 28%→26% · §1 "65% of waitlist"→75% (own-data 52+23).

**(C) Recomputation: 10 PASS / 2 FAIL → both fixed.**
- ✅ Compound risk: 152 suburbs, 0 mismatches vs independent recompute; bands Extreme 16 (top ~10.5%) · Critical 19 · High 94 · Moderate 20 · Low 3. Climate composites: 0 mismatches >1pt across 152.
- ✅ Waitlist sums (189,536 / 168,552 exact) · funding capacity $49.1B · timeline $35.2B/9 · yield 3,521/2,222 (−36.9%) · cost index 58.5 · ESG 26 metrics value↔numeric 0 mismatches · SA4 34 regions (top: NT Outback 85; bands C2/H25/M7/W0) · CHP implied SOMIH 15,300 ✓ · top-20 concentration 61.6%.
- 🔧 FAIL→fixed 1: **NSW flagship gap now computes $130,835** (TDC $578,660 · debt $192,374) — the $137k figure predated the Domain Mar-2026 rent update. Hero prose → **$579k / $192k / $131k**; historical comment annotated.
- 🔧 FAIL→fixed 2: HAFF R1 `by_sector` summed 13,746 (+97) → general bucket 7,946→7,849 (Σ=13,649 ✓); R3 `by_state` summed 21,250 vs the 21,350 target → NSW 6,400→6,500 (Σ=21,350 ✓); both marked HIVE-indicative splits.
- ⚠️ Noted, no action: population components (ni+nim) ≠ total growth in COVID-era years (worst 2020: −0.201M) — real ABS intercensal/timing noise, series uses official totals.

**Verdict: after 2 days of validation + this re-evaluation, every surfaced number is either primary-source-verified or explicitly labelled as a HIVE estimate. Zero unexplained numbers remain in the audited surface.**
...
**Roll-up:** each metric → rating vs benchmark → pillar score (HIVE judgement) → mean of 3 = **45**. ⚠️ Strict rating-averages (Leader 90 · Adequate 65 · Below Avg 45 · Lagging 25, re-run 2026-07-02 over the 26 metrics): **E 31 · S 43 · G 56 → mean ≈ 43** vs stated 32/48/56 → 45. After the NRSCH additions the optimism is now confined to **Social (+5)**; E and G are within ±1.5 of their rating-implied levels. Present as **HIVE ESG framework**, not an external rating.
