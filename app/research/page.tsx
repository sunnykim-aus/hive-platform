...
export default function ResearchPage() {
  ...
  const exportPolicyBrief = async () => {
    if (!result) return
    const date = new Date().toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" })
    const sources = result.sources.map(s =>
      `[${s.index}] ${s.title} · ${s.agency}${s.year ? ` · ${s.year}` : ""}${s.url ? `\n     ${s.url}` : ""} (match: ${(s.score * 100).toFixed(0)}%)`
    ).join("\n")

    const brief = [
      "HIVE POLICY ANALYSIS BRIEF",
      "─".repeat(60),
      `Policy: ${policy.event}`,
      ...
      "AI-synthesised from 681 indexed research reports. Verify against primary sources before use.",
    ].join("\n")
              ...
              <div>
                <div className="hive-card" style={{ marginBottom: 20 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                    ...
                    <button
                      onClick={exportPolicyBrief}
                      style={{
                        padding: "5px 14px", fontSize: "0.72rem", fontWeight: 700,
                        color: briefCopied ? "#0b1220" : "#f6c90e",
                        background: briefCopied ? "#f6c90e" : "rgba(246,201,14,0.06)",
